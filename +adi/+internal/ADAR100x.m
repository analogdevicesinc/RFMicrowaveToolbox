...
%                 newGains = zeros(size(Gains));
%                 for ii = 1:size(Gains, 1)
%                     for jj = 1:size(Gains, 2)
%                         newGains(ii, jj) = Gains(r(ii, jj), c(ii, jj));
%                     end
%                 end
%                 gain = ColumnWin.*newGains.*RowWin;
...
%         end
    ...
    methods
        function SaveRxBiasSet(obj, ChipIDIndx, ChIndx, BiasSettingIndex, LNABiasOff, LNABiasOn, BiasCurrentRx, BiasCurrentRxLNA)
            validateattributes( BiasSettingIndex, { 'double','single', 'uint32' }, ...
                { 'real', 'nonnegative','scalar', 'finite', 'nonnan', 'nonempty','integer'}, ...
                '', 'BiasSettingIndex');
            obj.setAttributeRAW(sprintf('voltage%d', ChIndx), 'bias_set_save', ...
                sprintf('%d, %d, %d, %d, %d', BiasSettingIndex, LNABiasOff, LNABiasOn, BiasCurrentRx, BiasCurrentRxLNA), ...
                false, obj.iioDevices{ChipIDIndx});
        end
        
        function SaveTxBiasSet(obj, ChipIDIndx, ChIndx, ...
                BiasSettingIndex, Ch1_PABiasOff, Ch2_PABiasOff, ...
                Ch3_PABiasOff, Ch1_PABiasOn, Ch2_PABiasOn, ...
                Ch3_PABiasOn, Ch4_PABiasOff, Ch4_PABiasOn, ...
                BiasCurrentTx, BiasCurrentTxDrv)
            validateattributes( BiasSettingIndex, { 'double','single', 'uint32' }, ...
                { 'real', 'nonnegative','scalar', 'finite', 'nonnan', 'nonempty','integer'}, ...
                '', 'BiasSettingIndex');
            obj.setAttributeRAW(sprintf('voltage%d', ChIndx), 'bias_set_save', ...
                sprintf('%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d', BiasSettingIndex, Ch1_PABiasOff, Ch2_PABiasOff, ...
                Ch3_PABiasOff, Ch1_PABiasOn, Ch2_PABiasOn, ...
                Ch3_PABiasOn, Ch4_PABiasOff, Ch4_PABiasOn, ...
                BiasCurrentTx, BiasCurrentTxDrv), true, obj.iioDevices{ChipIDIndx});
        end
